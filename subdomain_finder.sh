...
$TOOLS/subfinder.sh $1 
assetfinder $1 -subs-only | tee $1.assets.txt
cat 1.txt 2.txt $1.txt $1.assets.txt | sort -u | httprobe -prefer-https |sort -u |tee subdomains_$1.txt
cat subdomains_$1.txt | sed 's|^https://||g' | tee assets_$1.txt
